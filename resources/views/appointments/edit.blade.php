    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6 flex justify-between items-center">
            <h2 class="text-3xl font-bold text-gray-900">
                <i class="fas fa-edit mr-2 text-blue-600"></i>Edit Appointment
            </h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="date"
                            ...
                            value="{{ old('booking_date', $appointment->booking_date->format('Y-m-d')) }}" required>
                        @error('booking_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            const currentBookingTime = "{{ old('booking_time', $appointment->booking_time) }}";
                    ...
                    success: function(response) {
                        ...
                        if (response.slots && response.slots.length > 0) {
                            ...
                            response.slots.forEach(function(slot) {
                                const isSelected = slot.time === currentBookingTime ?
                                    'selected' : '';
                                $('#booking_time').append(
                                    '<option value="' + slot.time + '" ' + isSelected +
                                    '>' +
                            ...
                            // Add current booking time if not in available slots (allow keeping existing time)
                            if (currentBookingTime && !response.slots.find(s => s.time ===
                                    currentBookingTime)) {
                                $('#booking_time').append(
                                    '<option value="' + currentBookingTime + '" selected>' +
                                    currentBookingTime + ' (Current - may be full)' +
                                    '</option>'
                                );
                            }
                            ...
                            // Still allow current booking time even if no slots available
